...
cmd = '\nset +e\ncd /mnt/code\nexport PYTHONFAULTHANDLER=1\nmkdir -p /tmp/beam-train /outputs/beam-tmp-copy-smoke-b8\nnvidia-smi --query-gpu=name,memory.total --format=csv,noheader\necho TRAIN_TMP_THEN_COPY_BEGIN\npython -u train.py \\\n  --model edae_net \\\n  --seed 456 \\\n  --experiment-id beam_tmp_copy_smoke_eval1_b8 \\\n  --datasets DRIVE_patches \\\n  --data-root data \\\n  --output-root /tmp/beam-train \\\n  --wandb-mode disabled \\\n  --batch_size 4 \\\n  --epochs 1 \\\n  --loss bce_loss \\\n  --learning_rate 0.001 \\\n  --patches 10 \\\n  --patch_size 64 \\\n  --train_type patch \\\n  --type_split window \\\n  --num-workers 0 \\\n  --eval-batch-size 8 \\\n  --eval-every 1 \\\n  --eval-start-epoch 1 \\\n  --tf32 \\\n  --resume\nstatus=$?\necho TRAIN_TMP_STATUS=$status\nif [ "$status" = 0 ]; then\n  cp -a /tmp/beam-train/. /outputs/beam-tmp-copy-smoke-b8/\n  echo COPY_STATUS=$?\n  find /outputs/beam-tmp-copy-smoke-b8 -maxdepth 5 -type f | sed -n \'1,80p\'\nfi\nsleep 5\nexit $status\n'

pod = Pod(
    app="retinal-beam-tmp-copy-smoke",
    name="retinal-beam-tmp-copy-smoke-a10g",
    gpu="A10G",
    cpu=4,
    memory="32Gi",
    image=image,
