...
PROFILE="${MODAL_SECOND_PROFILE:-modal2}"
GPU_TYPE="${MODAL_GPU_TYPE:-A10:2}"
...
CONFIG="/tmp/${EXP_ID}_our_net_modal2.json"
PENDING="/tmp/${EXP_ID}_our_net_modal2.pending_tasks.json"
LOG="/tmp/${EXP_ID}_our_net_modal2.modal.log"
...
Pending: $PENDING
Log: $LOG
Each Modal task = our_net + one seed + all 7 datasets.
Inside each A10:2 container, train.py should show Dataset tasks to run: 7 and use GPU 0 + GPU 1.
INFO
