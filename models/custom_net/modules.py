...
class Conv_func_1(nn.Module):
    def __init__(self,in_channel,out_channel,kernel,padding='same',stride=1,dilation=1,norm=True,activation=True):
        ...
        self.router = nn.Sequential(
            nn.Linear(3,out_channel,bias=False),
            nn.LayerNorm(self.out_channel,elementwise_affine=False,bias=False),
            nn.Linear(out_channel,1,bias=False),nn.Sigmoid())
        self.experts =  nn.ModuleList([nn.Sequential(nn.Linear(self.in_channel,self.out_channel,bias=False),nn.LayerNorm(self.out_channel,elementwise_affine=False,bias=False)) for _ in range(self.kernel*self.kernel)])
    def forward(self,x):
        b,c,h,w = x.shape
...
class Residual_net_1(nn.Module):
    def __init__(self,in_channel,out_channel,kernel=3):
        ...
        self.change_feature = Conv_func_1(in_channel,out_channel,3,padding='same',activation=False)
    def forward(self,x):
        return self.change_feature(x)+self.feature(x)
...
class down_sampling(nn.Module):
    def __init__(self,in_channel,out_channel):
        ...
        self.down =nn.Conv2d(out_channel,out_channel,2,2,bias=False)
    def forward(self,x):
        out = self.out(x)
