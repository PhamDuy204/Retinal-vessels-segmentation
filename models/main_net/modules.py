...
class VesselRefineBlock(nn.Module):
    def __init__(self, channels, gamma_init=0.1):
        super().__init__()
        self.local = nn.Sequential(
            nn.Conv2d(channels, channels, 3, padding='same', bias=False),
            nn.GroupNorm(safe_group(channels, 8), channels),
            nn.ReLU(),
        )
        self.dw_d1 = nn.Conv2d(channels, channels, 3, padding=1, dilation=1,
                               groups=channels, bias=False)
        self.dw_d2 = nn.Conv2d(channels, channels, 3, padding=2, dilation=2,
                               groups=channels, bias=False)
        self.dw_d3 = nn.Conv2d(channels, channels, 3, padding=3, dilation=3,
                               groups=channels, bias=False)
    ...
    def forward(self, x):
        y = self.local(x)
        y = torch.cat((self.dw_d1(y), self.dw_d2(y), self.dw_d3(y)), 1)
        y = self.fuse(y)
...
class down_sampling(nn.Module):
    def __init__(self, in_channels, out_channels, in_size):
        ...
        self.refine = VesselRefineBlock(out_channels)
        self.down = nn.Conv2d(out_channels,out_channels,2,2,bias=False)

        init_module_weights(self)

    def forward(self, x):
        out = self.refine(self.proj(x))
