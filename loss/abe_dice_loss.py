...
class MultiScopeLoss(nn.Module):
    ...
    def forward(self,pred,truth):
        ...
        for i,wl in [(4,0.2),(8,0.3),(16,0.5)]:
            s=int(np.sqrt(i))
            unfold_pred=F.unfold(pred,i,stride=i//2).reshape(b,i,i,-1).permute(0,3,1,2)
            unfold_target = F.unfold(truth,i,stride=i//2).reshape(b,i,i,-1).permute(0,3,1,2)
            ...
            out = torch.where(sum_ > s,1,0)*dice_loss
...
class AbeDiceLoss(nn.Module):
    ...
    def forward(self,pred,truth):
        ...
        return 3*MultiScopeLoss()(pred,truth)+10*focal_loss+nn.BCELoss()(pred,truth)+nn.L1Loss()(pred.flatten(),truth.flatten())